...
return {
  {
    ...
    opts = function(_, opts)
      ...
      -- Actually, on Windows, <C-_> is is what neovim received when you press <C-/> on the keyboard.
      -- see: https://www.reddit.com/r/neovim/comments/ro2hm4/ctrl_keybinding_not_working/?tl=zh-hans
      maps.n = {
        ["<C-/>"] = opts.mappings.n["<Leader>/"],
        ["<C-_>"] = opts.mappings.n["<Leader>/"],
      }
      maps.x = {
        ["<C-/>"] = opts.mappings.x["<Leader>/"],
        ["<C-_>"] = opts.mappings.x["<Leader>/"],
      }
      maps.n["<Leader>/"] = false
      maps.x["<Leader>/"] = false
